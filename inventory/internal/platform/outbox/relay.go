...
// FOR UPDATE SKIP LOCKED позволяет безопасно запускать несколько реплик сервиса.
type Relay struct {
	pool      *pgxpool.Pool
	reader    Reader
	marker    Marker
...
// tick возвращает количество обработанных сообщений — вызывающий может решить,
// надо ли сразу крутить ещё один тик.
